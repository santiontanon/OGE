        ...
        for (int v = 0; v < embedding.length; v++) {
            ...
            for (OEElement oev : ev.embedding) {
                ...
                if (w > v) {
                    ...
                    if (DEBUG >= 1) {
                        System.out.println("Creating conector " + v + " -> " + w + " with bends " + oev.bends + ", " + oew.bends + " (" + OEElement.directionNames[oev.angle] + " - " + OEElement.directionNames[oew.angle] + ")");
                    }
...
//                                System.out.println("midpoints assigned to " + oev.v + " -> " + oew.v + " by method 1b");
                            ...
                            if (oev.angle == OEElement.LEFT) {
                                if (DEBUG >= 1) {
                                    System.out.println("  connector " + v + " to " + w + " with 2 bends (LEFT)");
                                }
                                ...
                                if (intermediate_x < startx) {
                                    if (DEBUG >= 1) {
                                        System.out.println("  connector with 2 bends (RIGHT), case 1");
                                    }
                                    x[idx] = startx + separation;
                                    y[idx] = starty;
                                    x[idx + 1] = startx + separation;
                                    ...
                                    if (oew.angle == OEElement.RIGHT && intermediate_x < endx) {
                                        if (DEBUG >= 1) {
                                            System.out.println("  connector with 2 bends (RIGHT), case 2");
                                        }
                                        x[idx] = endx + separation;
                                        y[idx] = starty;
                                        x[idx + 1] = endx + separation;
                                        ...
                                        if (DEBUG >= 1) {
                                            System.out.println("  connector with 2 bends (RIGHT), case 3");
                                        }
                                        x[idx] = intermediate_x;
                                        y[idx] = starty;
                                        x[idx + 1] = intermediate_x;
                            ...
                            if (oev.angle == OEElement.LEFT) {
                                if (DEBUG >= 1) {
                                    System.out.println("  connector with 3 bends (LEFT)");
                                }
                                ...
                                if (DEBUG >= 1) {
                                    System.out.println("  connector with 3 bends (RIGHT)");
                                }
                                double tmpx = intermediate_x;
                                if (intermediate_x < startx) {
                                    tmpx = startx + separation;
                                ...
                                if (DEBUG >= 1) {
                                    System.out.println("  connector with 3 bends (UP/DOWN)");
                                }
                                double tmpx = intermediate_x;
                                if (oew.angle == OEElement.LEFT) {
                                    if (intermediate_x > endx) {
    ...
    public boolean fixNonOrthogonalEdge(Pair<Integer, Integer> edge, boolean desperate) throws Exception {
        ...
        if (DEBUG >= 1) {
            System.out.println("fixNonOrthogonalEdges: " + edge);
        }
        ...
        if (v != null) {
            ...
            if (DEBUG >= 1) {
                System.out.println("  looking for " + v.v + " -> " + target);
            }
            if (v.v == target) {
                throw new Exception("fixNonOrthogonalEdges: looking for " + v.v + " -> " + target);
            }
        ...
        if (w != null) {
            ...
            if (DEBUG >= 1) {
                System.out.println("  looking for " + w.v + " -> " + target);
            }
            if (w.v == target) {
                throw new Exception("fixNonOrthogonalEdges: looking for " + w.v + " -> " + target);
            }
        ...
        if (DEBUG >= 1) {
            System.out.println("  v: " + oev);
        }
        if (DEBUG >= 1) {
            System.out.println("  w: " + oew);
        }
        ...
        if ((oev == null || (oev.angle == OEElement.LEFT || oev.angle == OEElement.RIGHT))
            ...
            if (DEBUG >= 1) {
                System.out.println("  V range of " + edge.m_a + ": " + range_v);
            }
            if (DEBUG >= 1) {
                System.out.println("  V range of " + edge.m_b + ": " + range_w);
            }
            if (range_v.m_a <= range_w.m_b
                ...
                if (fits) {
                    ...
                    if (DEBUG >= 1) {
                        System.out.println("Fixed Y to " + new_y);
                    }
                    return true;
                }
            }
            if (desperate) {
                if (DEBUG >= 1) {
                    System.out.println("Edge does not fit!");
                }
        ...
        if ((oev == null || (oev.angle == OEElement.UP || oev.angle == OEElement.DOWN))
            ...
            // if the ranges overlap, then move them to the center of the overlap:
            if (DEBUG >= 1) {
                System.out.println("  H range of " + edge.m_a + ": " + range_v);
            }
            if (DEBUG >= 1) {
                System.out.println("  H range of " + edge.m_b + ": " + range_w);
            }
            if (range_v.m_a <= range_w.m_b
                    && range_w.m_a <= range_w.m_b) {
...
//                if (edgeFits(new_x,y[edge.m_a], new_x, y[edge.m_b])) {
                ...
                if (fits) {
                    ...
                    if (DEBUG >= 1) {
                        System.out.println("Fixed X to " + new_x);
                    }
                    return true;
                }
            }
        ...
        if (DEBUG >= 1) {
            System.out.println("fixNonOrthogonalEdges: can't fix " + edge.m_a + "->" + edge.m_b);
        }
        return false;
    }
    ...
    public Pair<Double, Double> nodeHorizontalWiggleRoom(int vertex, List<Integer> nodeGroup) {
        ...
        if (DEBUG >= 1) {
            System.out.println("  Node " + vertex + "'s vertical connections:" + nodeGroup);
        }

        // Find the wiggleRoom of all of them:
        List<Pair<Double, Double>> wiggleRooms = new LinkedList<Pair<Double, Double>>();
        for (Integer v : nodeGroup) {
            wiggleRooms.add(nodeHorizontalWiggleRoomSingleNode(v));
        }
    ...
    public Pair<Double, Double> nodeVerticalWiggleRoom(int vertex, List<Integer> nodeGroup) {
        ...
        if (DEBUG >= 1) {
            System.out.println("  Node " + vertex + "'s horizontal connections:" + nodeGroup);
        }

        // Find the wiggleRoom of all of them:
        List<Pair<Double, Double>> wiggleRooms = new LinkedList<Pair<Double, Double>>();
        for (Integer v : nodeGroup) {
            wiggleRooms.add(nodeVerticalWiggleRoomSingleNode(v));
        }

        // Compute the intersection:
        Pair<Double, Double> result = wiggleRooms.remove(0);
        if (DEBUG >= 1) {
            System.out.println("  Initial wiggle room: " + result);
        }
        while (!wiggleRooms.isEmpty()) {
            Pair<Double, Double> tmp = wiggleRooms.remove(0);
            if (DEBUG >= 1) {
                System.out.println("  Merging with: " + tmp);
            }
            result.m_a = Math.max(result.m_a, tmp.m_a);
            result.m_b = Math.min(result.m_b, tmp.m_b);
        }
    ...
    public void gridAlign(double step) {
        ...
        for (double tmp : y) {
            if (!yvalues.contains(tmp)) {
                yvalues.add(tmp);
            }
        }
        for (double tmp : x) {
            if (!xvalues.contains(tmp)) {
                xvalues.add(tmp);
            }
        }
                            ...
                            if (edges[i2][j2] || edges[j2][i2]) {
                                ...
                                if (minx1 <= maxx2
                                    ...
                                    if (!silent) {
                                        ...
                                        System.err.println("indexes " + nodeIndexes[i1] + "->" + nodeIndexes[j1] + " crosses with " + nodeIndexes[i2] + "->" + nodeIndexes[j2]);
